...
import fire.pb.timer.ShutdownServerPostTask;
...
public class Stopper implements StopperMXBean {
	...
	static ShutdownServerPostTask task = null;
	...
	public void setStopTime(long seconds) {
		...
		try {
			...
			stopTime = Calendar.getInstance().getTimeInMillis() + seconds
					* 1000L;
			task = new ShutdownServerPostTask();
			mkdb.Executor.getInstance().scheduleAtFixedRate(task, 0, 1, TimeUnit.MINUTES);
