...
public class COneKeyTeamMatch extends __COneKeyTeamMatch__ {
	...
	protected void process() {
		...
		if (roleid < 0)
			return;
		/*Âä†ÈîÅÈ°∫Â∫è team->roleid->match by changhao*/
		mkdb.Procedure onekeyteammatch = new mkdb.Procedure()
		{
			...
			protected boolean process()
			{
				...
				//Â¶ÇÊûúÊòØÂÖ¨‰ºöÂèëÈÄÅÔºåÁúãÁúãÊòØÂê¶ÊúâÂÖ¨‰º? by changhao
				...
				if (teamid == null)
				{
					...
					TeamManager.logger.error("COneKeyTeamMatch:Ëá™Â∑±‰∏çÊòØÈòüÈïø "+roleid);
				...
				Team team = null;
				team = TeamManager.getTeamByTeamID(teamid);	//ÊúâÈòüÂÖàÊääÈòü‰ºçÈîÅ‰Ωè by changhao	
				...
				if (fire.pb.fushi.Module.GetPayServiceType() == 1) //ÁÇπÂç°ÊúçÂä°Âô? by changhao
				{
					...
					if (config == null)
					{
						...
						TeamManager.logger.error("COneKeyTeamMatch:ÁõÆÊ†áIDÈîôËØØ "+roleid);
				...
				if (delta < 0)
				{
					...
					TeamManager.logger.info("COneKeyTeamMatch:‰∏?ÈîÆÂñäËØùÊó∂Èó¥Èó¥Èö?60Áßí„?? "+roleid);
				...
				if (msg1 == null)
				{
					TeamManager.logger.error("COneKeyTeamMatch:Êâæ‰∏çÂà∞Â≠óÁ¨¶‰∏≤ "+roleid);
					return false;						
				}
				*/
	...
	public int channeltype; // ∆µµ¿¿‡–Õ 14◊È∂”…Í«Î 1µ±«∞∆µµ¿ 4∞Ô≈… 5 ¿ΩÁ∆µµ¿ by changhao
	public java.lang.String text; // ∑¢ÀÕµƒŒƒ±æ by changhao
