...
public class CInviteJoinTeam extends __CInviteJoinTeam__ {
	...
	protected void process()
	{
		...
		if(prole.getProperties().getCruise() > 0) {
			TeamManager.logger.info("CInviteJoinTeam:闂傚倸鍊搁崐宄懊归崶褏鏆﹂柣銏㈩焾绾惧鏌ｉ幇顒佹儓缂侊拷閸曨厽鍠愰柣妤�鐗嗙粭鎺旂棯閹规劦鍤欓柍瑙勫灴閹晠宕ｆ径瀣�风紓浣哄亾閸庡磭绱炴繝鍥х畺婵°倕鎳忛弲鏌ュ箹缁櫢鎷烽幇浣规祰闂備浇顕х�涒晝鍠婂鍛殕闁归棿绀侀拑鐔烘喐瀹ュ缍栨繝闈涱儛閺佸棝鏌ㄩ悢鍓佺煓闁轰焦鎹囬幐濠冨緞瀹�锟芥闂備浇顕栭崹鏉棵洪銏犳瀬闁瑰墽绮弲鎼佹煥閻曞倹瀚�" + invitedRoleId + "闂傚倸鍊峰ù鍥敋瑜庨〃銉х矙閸柭わ拷鍧楀级閸碍娅嗛柛蹇旂矒閺屾盯顢曢敐鍡欘槬缂備緤鎷烽柛顐ゅ枔缁★拷闂傚倸鐗婃笟妤呭磿閹扮増鐓曢悗锝庝憾濡拷闂佸搫鏈惄顖炪�侀弴銏℃櫜闁糕剝鐟Σ浼存⒒娴ｄ警鐒炬い鎴濇瀹曟繂鈻庨幘瀹犳憰闂侀潧鐗嗛ˇ浼此夐崼銉︾厓闁告繂瀚敓鑺ユ倐椤㈡棃鏁撻敓锟�??,婵犵數濮烽弫鍛婃叏閻㈠壊鏁婇柡宥庡幖缁愭淇婇妶鍛殶缂佹唻绠撻弻娑滎槼妞ゃ劌鎳愭竟鏇°亹閹烘挾鍘搁悗瑙勬惄閸犳牠寮甸鍌︽嫹闂堟稓鐒告慨濠呮缁瑩骞愭惔銏″缂傚倷绀侀鍡涘箲閸ヮ剙鏄ラ柕蹇嬪�曠猾宥夋煕瀹�瀣閻庢俺顫夐〃銉╂倷閼碱兛铏庨梺鍛婃⒐瀹�鎼佸蓟閿濆憘鏃堟晸閽樺）娑樷攽閸℃瑦娈鹃梺闈涚箞閸婃洜绮荤紒妯镐簻闁哄啫鍊瑰▍鏇㈡煕濮楋拷娴滆泛螞閸涙惌鏁冮柕蹇婂墲閹瑥顪冮妶鍡楃仴闁硅櫕锚閻ｅ嘲鈹戠�ｎ偅娅嗛梺鐟扮摠鐢偟绮诲鑸电厽闁绘ê寮堕崢鍌炴煕閺囥劌浜藉ù纭锋嫹");
		...
		mkdb.Procedure createTeamP = new mkdb.Procedure()
		{
			...
			@Override /*lock濠电姷鏁告慨鐑姐�傞鐐潟闁哄洢鍨圭壕濠氭煙鏉堝墽鐣辩痪鎯х秺閺岋繝宕堕妷銉т患缂備胶濮鹃～澶愬Φ閸曨垰绠涢柍杞扮筏閹峰嘲鈽夊▎鎰伎闂佽鍎兼慨銈夋偂閵夆晜鐓熼柡鍥╁仜閿熻棄婀遍敓鑺ョ啲閹凤拷 team->roleid->match by changhao*/
...
//					psend(inviterRoleId, new STeamError(TeamError.BeingInvited));
				...
				else if(team != null)
				{
					...
					if(!checkTeamNotFull(team))
					{
						...
						TeamManager.logger.info("FAIL:CInviteJoinTeam:闂傚倸鍊搁崐鎼佸磹閹间礁纾瑰瀣椤愪粙鏌曢崼婵愭Ц缂佺媭鍨堕弻銊╂偆閸屾稑顏�?闂傚倸鍊搁崐宄懊归崶褏鏆﹂柛顭戝亝閸欏繘鏌℃径瀣婵炲樊浜滄儫闂佸疇妗ㄩ懗鍫曀囪閳规垿鍩ラ崱妤冧户缂傚倸绉寸粔褰掑极閹邦厼绶為悗锝庡亝閻濇洟姊绘担钘壭撻柨姘亜閿旇法鐭欑�规洜鏁绘俊鑸靛緞鐎ｎ亷鎷烽崹顐闁绘劘灏欐禒銏ゆ煕閺冿拷绾板秵绌辨繝鍥舵晝闁挎繂瀛╅悾鐑芥⒑闁稓鈹掗柛鏃�鍨块妴浣肝旈崨顔芥婵炲濮撮鍡欑不閹烘鈷掑ù锝堝Г绾爼鏌涢敐蹇曠暤妤犵偛绻橀弫鎾绘晸閿燂拷?,TeamId: "+inviterTeamId);
					}
					else if(!checkTeamFilter(team, invitedRoleId))
					{
						TeamManager.logger.info("FAIL:CInviteJoinTeam:TeamFilter闂傚倸鍊搁崐鎼佸磹妞嬪海鐭嗗〒姘炬嫹妤犵偛顦甸弫鎾绘偐閸愯弓绨绘俊鐐�栫敮鎺楀磹缂佹鈻旂�广儱顦伴悡娆撳级閸繂鈷旈柣锝堜含缁辨帡骞囬褎鐤侀梺鍝勮閸旀垿骞冮妶澶婄＜婵炴垶锕╂导锟�,TeamId: "+inviterTeamId);
					}
...
//					}
						...
						else if (!team.isTeamLeader(inviterRoleId)) //濠电姷鏁告慨鐑藉极閸涘﹥鍙忛柣鎴濐潟閿熻棄鍊圭粋鎺旓拷锝庡亝濞呫垽姊洪懞銉冾亪藝椤栫偛姹查梺顒�绉甸悡銉╂煟閺冨倻甯涙い銉ヮ槺缁辨挸顓奸崪鍐惈濡ょ姷鍋涢崯顐ョ亽闁荤姴娲╃亸娆戠玻閻愮儤鈷戠憸鐗堝笒娴滀即鏌涘锟界粻鏍х暦濠靛鏅搁柨鐕傛嫹?闂傚倸鍊搁崐宄懊归崶褏鏆﹂柛顭戝亝閸欏繘鏌℃径瀣婵炲樊浜滄儫闂佸疇妗ㄥù鍥箺閺囥垺鈷戦悹鎭掑妼濞呮劙鏌熼崙銈嗗?闂傚倸鍊搁崐椋庣矆娓氾拷楠炲鍩勯崘顏嗘嚌濠德板�曢幊搴ｇ矆閸喓绠鹃柟瀛樼懃閻忣亪鏌涚�Ｑ冨⒉缂佺粯绻堝Λ鍐ㄢ槈閸楃偛澹夐梻浣虹帛閹稿鎯勯鐐茶摕婵炴垯鍨归崡鎶芥煏韫囧﹤澧查梺娆惧弮濮婅櫣鎷犻弻銉у椽闂傚倸瀚�氼厾绮氭潏銊х瘈闁告洦鍓欓悵浼存偡濠婂啰绠婚柟顕�绠栧畷褰掝敃閿濆懎浼庡┑鐘垫暩婵鎹㈤幋鐙呮嫹濮樼偓瀚�? by changhao
						{
							snd.op = 2;
...
//						Message.sendMsgNotify(inviterRoleId, 160202, null);
						TeamManager.logger.info("闂傚倸鍊搁崐鎼佸磹閻戣姤鍤勯柛鎾茬閸ㄦ繃銇勯弽銊с�掗柣婵嗙埣閺岋繝宕堕妷銉т患闂佹眹鍊楅崑鎾舵崲濞戙垹绠ｆ繛鍡楃箳閸旀挳姊烘潪鎵槮闁挎洦浜濠氬Ω閳哄倸浜為梺绋挎湰缁嬫垿顢旈敓锟�(roleId=" + invitedRoleId+")invite濠电姷鏁告慨鐑藉极閸涘﹥鍙忓ù鍏兼綑閸ㄥ倿鏌ｉ幘宕囧哺闁哄鐗楃换娑㈠箣閻愰潧鏆￠梺鐟板槻閸㈡煡锝炲┑瀣拻閻庨潧鎲￠弲濂告⒒娴ｇ鎮戞俊鐐跺Г缁傚秹宕奸弴姗堟嫹娴ｈ倽鏃堝川椤撶媭妲规俊鐐�栭崝鎴﹀磹閵堝憘锝夋嚋閸忓摜绠氶梺缁樺姦娴滄粓鏁撻懞銉у⒌闁诡啫鍏炬棃宕橀鍡欏幆闂備礁婀遍崕銈夈�冮崨顖滀笉妞ゆ牜鍋為悡鏇熶繆閵堝懎鏆欏ù婊嗩潐娣囧﹪骞撻幒鏂库叺闂佸搫琚崝鎴﹀箖閵堝纾兼繛鎴烇供娴硷拷?,濠电姷鏁告慨鐑藉极閹间礁纾婚柣鎰惈閸ㄥ倿鏌涢锝嗙缂佺姳鍗抽弻娑樷攽閸曨偄濮㈤梺娲诲幗閹瑰洭寮婚敐澶婄闁挎繂妫Λ鍕磽娓氬﹥瀚归梺绋挎湰缁嬫劗鎹㈤崱妯镐簻闁哄秲鍔庣粻鎾趁瑰鍐╄础闁汇儺浜獮鍡氼槹闁稿鍨介弻锛勪沪閸撗勫垱閻庢鍠楅幐铏繆閹间礁唯鐟滃矂鎮芥繝姘厽闁绘劕顕崯鏌ユ煙閸戙倖瀚�");
					}
					else if (fire.pb.buff.Module.existState(inviterRoleId,
							BuffConstant.StateType.STATE_INSTANCE_ZONE)) {
//						Message.sendMsgNotify(inviterRoleId, 160202, null);
						TeamManager.logger.info("闂傚倸鍊搁崐鎼佸磹閻戣姤鍤勯柛鎾茬閸ㄦ繃銇勯弽銊с�掗柣婵嗙埣閺岋繝宕堕妷銉т患闂佹眹鍊楅崑鎾舵崲濞戙垹绠ｆ繛鍡楃箳閸旀挳姊烘潪鎵槮闁挎洦浜濠氬Ω閳哄倸浜為梺绋挎湰缁嬫垿顢旈敓锟�(roleId=" + inviterRoleId+")invite濠电姷鏁告慨鐑藉极閸涘﹥鍙忓ù鍏兼綑閸ㄥ倿鏌ｉ幘宕囧哺闁哄鐗楃换娑㈠箣閻愰潧鏆￠梺鐟板槻閸㈡煡锝炲┑瀣拻閻庨潧鎲￠弲濂告⒒娴ｇ鎮戞俊鐐跺Г缁傚秹宕奸弴姗堟嫹娴ｈ倽鏃堝川椤撶媭妲规俊鐐�栭崝鎴﹀磹閵堝憘锝夋嚋閸忓摜绠氶梺缁樺姦娴滄粓鏁撻懞銉у⒌闁诡啫鍏炬棃宕橀鍡欏幆闂備礁婀遍崕銈夈�冮崨顖滀笉妞ゆ牜鍋為悡鏇熶繆閵堝懎鏆欏ù婊嗩潐娣囧﹪骞撻幒鏂库叺闂佸搫琚崝鎴﹀箖閵堝纾兼繛鎴烇供娴硷拷?,濠电姷鏁告慨鐑藉极閹间礁纾婚柣鎰惈閸ㄥ倿鏌涢锝嗙缂佺姳鍗抽弻娑樷攽閸曨偄濮㈤梺娲诲幗閹瑰洭寮婚敐澶婄闁挎繂妫Λ鍕磽娓氬﹥瀚归梺绋挎湰缁嬫劗鎹㈤崱妯镐簻闁哄秲鍔庣粻鎾趁瑰鍐╄础闁汇儺浜獮鍡氼槹闁稿鍨介弻锛勪沪閸撗勫垱閻庢鍠楅幐铏繆閹间礁唯鐟滃矂鎮芥繝姘厽闁绘劕顕崯鏌ユ煙閸戙倖瀚�");
					}
					else
					{
						if(!checkMap()){
...
//					}
	...
	private static int checkPvP(long inviterRoleId, long invitedRoleId) {
		// 闂傚倸鍊搁崐鎼佸磹閹间礁纾瑰瀣椤愪粙鏌曢崼婵愭Ц缂佺媭鍨堕弻銊╂偆閸屾稑顏�?闂傚倸鍊搁崐宄懊归崶褏鏆﹂柛顭戝亝閸欏繘鏌℃径瀣婵炲樊浜堕弫鍥煏韫囨洖啸闁告繃顨婂缁樼瑹閸パ冾潻缂備礁顦伴幐鍐茬暦瑜版帒纾兼繛鎴炵懃瀵灝鈹戦埥鍡楃仭閻庣瑳鍛厹闁告挆宥嗗鐟滅増甯楁禍銈夋煙閸戙倖瀚�?
