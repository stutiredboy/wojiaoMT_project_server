...
public class CMovePetColumn extends __CMovePetColumn__ {
	...
	protected void process() {
		...
		if (!NpcServiceManager.getInstance().hasServiceByNpcKey(roleid, npckey, NpcServices.PET_STORE))
			return;

		if (srccolumnid == PetColumnTypes.PET) {
			if (!fire.pb.pet.Pet.isInBattle(roleid, petkey))
				return;
		}

		PMovePet proc = new PMovePet(srccolumnid, petkey, dstcolumnid, npckey, roleid);
		proc.submit();
	}
