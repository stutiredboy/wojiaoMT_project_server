...
public class PvP3Stage {
	...
	public static class Start extends PvPStageCommon.Start {
		...
		public void onEnter(PvPControl c) {
			...
			// 拉人
			RoleManager.getRolesByConditions(-1, PvP3Helper.ENTER_MIN_LEVEL,
					-1, -1, -1, new IGetRolesCallBack() {
						...
						public void process(List<Long> roleIds) {
							...
							send.mapid = 0;
							send.minimal = 0;
							send.summontype = TransmitTypes.pvp3;
	...
	public static class Fight extends PvPStageCommon.Fight {
		...
		public void onTick(PvPControl c) {
			if (c.isFightTime()) {
				// 进行匹配
				for (int grade : PvP3Helper.getAllRaceIds()) {
					new PPvP3PairBattleFromQueue(grade).submit();
				}
			}
		}
	}
