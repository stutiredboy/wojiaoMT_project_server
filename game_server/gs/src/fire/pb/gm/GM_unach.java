...
public class GM_unach extends GMCommand {

	@Override
	boolean exec(String[] args) {
		if (args.length < 1) {
			sendToGM("用法为：//unach roleid 成就id");
			return false;
		}
		...
		new mkdb.Procedure() {
			...
			protected boolean process() throws Exception {
				...
				CourseColumn courseCol = fire.pb.course.CourseManager.getCourse(roleid, false);
				courseCol.unachieved(achieveid);
