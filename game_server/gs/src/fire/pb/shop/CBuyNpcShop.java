...
//			if (npcid < 0) {
//				xdb.Trace.info("Êâæ‰∏çÂà∞ËØ•npc id=" + npckey);
//				return;
//			}
//		}
		...
		if (null == goods) {
			mkdb.Trace.info("Ê≤°ÊúâËØ•ÂïÜÂìÅ[" +  goodsid  + "]");
			return;
		}
		
		// Âà§Êñ≠ËØ•npcÊòØÂê¶ÊúâËØ•Áâ©ÂìÅ
		if (buytype == ShopBuyType.NORMAL_SHOP) {
			fire.pb.shop.SNpcSale ns = fire.pb.npc.NpcManager.getInstance().getNpcSale(shopid);
			if (ns == null || !ns.getGoodsids().contains(goodsid)) {
				mkdb.Trace.info("NPCÂπ∂‰∏çÂá∫ÂîÆËØ•Áâ©Âì?.shopid:" + shopid + "goodsid:" + goodsid);
				return;
			}
		}
	...
	public int shopid; // …ÃµÍ–Ú∫≈
	public int goodsid; // …Ã∆∑id
	public int num; // π∫¬Ú ˝¡ø
	public int buytype; // π∫¬Ú¿‡–Õ
