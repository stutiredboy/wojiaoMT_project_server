...
public class SAddTeamMember extends __SAddTeamMember__ {
	...
	protected void process() {
		...
		if (ui == null || ui.getLoginRole() == null || ui.getLoginRole().getTeam() == null)
			return;
		for (fire.pb.team.TeamMemberBasic member : memberlist) {
			if (ui.getLoginRole().getTeam().contains(member.roleid))
				continue;
			ui.getLoginRole().getTeam().addRole(member.roleid, member.state);
		}
	}
