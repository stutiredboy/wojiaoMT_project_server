...
public class SRoleList extends __SRoleList__ {
	...
	protected void process() {
		...
		//mkdb.Trace.error("角色列表是否为空----------------------："+roles.isEmpty());
		if (roles.isEmpty()) {
			// 这里先随机一个角色名称
			CRequestName cRequestName = new CRequestName();
			cRequestName.sex = 1;
			ins.send(cRequestName);
		} else {
			ins.send(new CEnterWorld(roles.iterator().next().roleid, 2));
		}
	}
	...
	public SRoleList(long _prevloginroleid_, byte _prevroleinbattle_, java.util.LinkedList<fire.pb.RoleInfo> _roles_, int _gacdon_) {
