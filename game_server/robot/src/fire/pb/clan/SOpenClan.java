...
public class SOpenClan extends __SOpenClan__ {
	...
	protected void process() {
		...
		if (ui == null||ui.getLoginRole()==null)
			return;
		int school = ui.getLoginRole().school;
		// mkdb.Trace.error("school-------------------------"+school);
		int id = 0;
		switch (school) {
		case 11:
			id = 5003;
		...
		// mkdb.Trace.error("school-----------id--------------"+id);
		ins.send(new CSendCommand("//standard " + id));
