...
public class Pet {
	...
	public fire.pb.Pet getProtocolPet() {
		...
		if(roleId < 1)
		{
			...
			Map<Integer, Integer> allequipList = getEquipList();
			for(Map.Entry<Integer, Integer> entry : allequipList.entrySet())
			{
				Module.logger.error("****************宠物装备ID++++++++111111+++++++"+entry.getKey()+"-------------"+entry.getValue());
				pet.equiplist.put(entry.getKey(),entry.getValue());
			}
		}
		else
		{
	...
	public Map<Integer, Integer> getEquipList()
	{
	...
	public boolean addEquipItem(int pos,int itemid)
	{
		int maxEquipNum = getEquipMaxNum();
		Map<Integer, Integer> equipList = getEquipList();
	...
	{
		Map<Integer, Integer> equipList = getEquipList();
		for (Iterator<Integer> iterator = equipList.values().iterator(); iterator.hasNext();) {
