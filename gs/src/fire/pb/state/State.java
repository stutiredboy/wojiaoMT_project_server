...
public abstract class State implements IState{
	...
	public final static int END_TRUSTEESHIP_STATE = 10;	// 结束战斗托管状态
	...
	public final static int TRIGGER_CLIENT_LOCK_SCREEN = 8;	//客户端锁屏
	...
	{
		StateManager.logger.error(new StringBuilder("roleId=").append(roleId).append(" 角色状态").append(this.getClass().getCanonicalName()).append("转移失败，trigger = ").append(trigger));
	}
	protected void enterErrorLog(int oldstate, int trigger)
	{
		StateManager.logger.error(new StringBuilder("roleId=").append(roleId).append(" 角色状态").append(this.getClass().getCanonicalName()).append("转移失败，oldstate = ").append(oldstate).append("，trigger = ").append(trigger));
